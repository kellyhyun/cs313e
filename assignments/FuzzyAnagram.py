...
def divide_conquer (grid):
 max_value = divide_conquer_helper(grid, 0, 0, 0)
...
def divide_conquer_helper(grid, col_position, sum, n):
 if n == len(grid)-1:
   return grid[len(grid)-1][col_position]
 else:
   # What if they are the same??
   if divide_conquer_helper(grid, col_position, sum, n+1) > divide_conquer_helper(grid, col_position+1, sum, n+1):
    return divide_conquer_helper(grid, col_position, sum, n+1) + grid[n][col_position]
   return divide_conquer_helper(grid, col_position+1, sum, n+1) + grid[n][col_position]
