...
#  Student Name: Joon Kim

#  Student UT EID: jk45873

#  Partner's Name: Kelly Hyun (Unique #: 52600)

#  Partner's UT EID: sh52679
...
def groupSum(start, nums, target):
   if target == 0:
       return True
   if target < 0 or (start >= len(nums) and target != 0):
       return False

   # can choose to either include the number in the group or not
...
def groupSum6(start, nums, target):
   ...
   # always includes 6 in the group
   if nums[start] == 6:
...
def groupNoAdj(start, nums, target):
   ...
   # can choose to either include the number in the group and skip the next number or not
...
def groupSum5(start, nums, target):
   ...
   # always includes multiple of 5 in the group
   if nums[start] % 5 == 0:
       # does not include 1 if it comes right after a multiple of 5
...
def groupSumClump(start, nums, target):
   ...
   if start <= len(nums) - 2 and nums[start] == nums[start+1]:
       num_skips = 1
       # checks if subsequent numbers are equal
       for i in range(start+1, len(nums)):
           if nums[i+1] == nums[i]:
               num_skips += 1
           else:
               break
       # either includes all the subsequent, equal numbers or excludes all
       return groupSumClump(start+num_skips+1, nums, target-nums[start]*(num_skips+1)) or groupSumClump(start+num_skips+1, nums, target)
   else:
...
def splitArray(nums):
   ...
   for i in nums:
       sum += i
   # target sum of each group will be half the sum of all numbers because both groups have the same sum
   target = sum / 2

   return splitArrayHelper(0, nums, target)
...
def splitArrayHelper(start, nums, target):
   ...
   # can choose to either include the number in one group or not in that group
   return splitArrayHelper(start+1, nums, target) or splitArrayHelper(start+1, nums, target-nums[start])
...
def splitOdd10Helper(start, nums, sum1, sum2):
   # returns true if the sum of one group is a multiple of 10 and the sum of the other group is odd
   if start >= len(nums) and ((sum1 % 10 == 0 and sum2 % 2 == 1) or (sum1 % 2 == 1 and sum2 % 10 == 0)):
       return True
   if start >= len(nums):
       return False

   # can choose to either include the number in one group or the other
...
def split53(nums):
   ...
   return split53Helper(0, nums, target, target)



def split53Helper(start, nums, target_1, target_2):
   if start >= len(nums) and target_1 == 0 and target_2 == 0:
       return True

   if target_1 < 0 or target_2 < 0 or (start >= len(nums) and (target_1 != 0 or target_2 != 0)):
       return False

   # assigns multiples of 5 into one group
   if nums[start] % 5 == 0:
       return split53Helper(start+1, nums, target_1-nums[start], target_2)
   # assigns multiples of 3, which are not multiples of 5, into the other group
   elif nums[start] % 3 == 0 and nums[start] % 5 != 0:
       return split53Helper(start+1, nums, target_1, target_2-nums[start])
   # assigns other numbers to either one group or the other
   else:
       return split53Helper(start+1, nums, target_1-nums[start], target_2) or split53Helper(start+1, nums, target_1, target_2-nums[start])
