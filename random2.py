...
def is_balanced(string):
    ...
    for char in string:
        if (char == "(" or char == "[" or char == "{" or char == "<"):
            ...
            if (char == ")" and stack.peek() == "(") or (char == "]" and stack.peek() == "[") or (
                    char == "}" and stack.peek() == "{") or (char == ">" and stack.peek() == "<"):
                stack.pop()
            else:
                return (False)
...
def pancake_sort(sizes):
    if (len(sizes) == 1):
        ...
        sizes = flip_stack + sizes[
                             flapjack_index + 1:]  # concatenate flipped stack with the other pancakes that weren't flipped
...
def main():
    ...
    [myList.insert_in_order(random3.randint(-size, size)) for i in range(size)]
    print(myList)
    ...
    pancake_amt = 100
    pancakes = [random3.randint(1, pancake_amt) for i in range(pancake_amt)]
    s_pancakes = pancake_sort(pancakes)
    print(
        f"pancake_sort([{pancakes[0]}, ..., {pancakes[-1]}]) => [{s_pancakes[0]}, {s_pancakes[1]}, ..., {s_pancakes[-2]}, {s_pancakes[-1]}]\n")
